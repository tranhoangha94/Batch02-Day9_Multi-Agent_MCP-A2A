...
def check_statute_of_limitations(case_type: str) -> str:
    ...
    """
    limits = {
        "contract": "4 năm (UCC § 2-725 cho hợp đồng mua bán hàng hóa)",
        "tort": "2-3 năm tùy bang (personal injury thường 2 năm)",
        "property": "3-10 năm tùy loại tài sản và jurisdiction",
    }
    key = case_type.lower().strip()
    for name, period in limits.items():
        if name in key:
            return f"Thời hiệu khởi kiện ({case_type}): {period}"
    return (
        f"Không có dữ liệu cho '{case_type}'. "
        f"Các loại hỗ trợ: {', '.join(limits)}"
    )


TOOL_MAP = {
    "search_legal_knowledge": search_legal_knowledge,
    "check_statute_of_limitations": check_statute_of_limitations,
}
